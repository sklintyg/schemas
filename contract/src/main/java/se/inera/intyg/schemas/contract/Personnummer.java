...
public final class Personnummer implements Serializable {
  ...
  /**
   * Returns a Personnummer, iff the String is has a valid format. Valid formats are:
   *
   * <ul>
   *   <li>^(19|20)[0-9]{6}[-+]?[0-9]{4}$
   *   <li>^[0-9]{6}[+-]?[0-9]{4}$
   * </ul>
   *
  ...
  public static Optional<Personnummer> createPersonnummer(String personnummer) {
    try {
      ...
      LOG.info(
          String.format(
              "Can not create a Personnumer object with '%s'. Returning Optional.empty()",
              personnummer));
  ...
  /**
   * Returns true if the actual control digit is the one calculated by
   * Personnummer#calculateControlDigit.
   *
  ...
  /**
   * If we need to check the syntactic validity of a personnummer, this function can be used to
   * calculate the "control digit" of a swedish personnummer.
   *
   * @return The expected control digit. If the personnummer was syntactically invalid, -1 will be
   *     returned.
   */
  ...
  /**
   * Will return the hashed personnummer to make sure the real personnummer is not accidentally
   * logged.
   *
  ...
  public String toString() {
    assert false
        : "Don't use implicit toString. Use explicit getPersonnummerHash or getPersonnummer instead.";
  ...
  /**
   * Returns the normalized personnummer if it is valid, otherwise returns the originally set
   * personnummer.
   */
  ...
  private String normalizePnr() throws InvalidPersonNummerException {
    ...
    if (originalPnr.matches("^[0-9]{6}[+-]?[0-9]{4}$")) {
      return getCenturyFromYearAndSeparator(originalPnr)
          + originalPnr.replace("-", "").replace("+", "");
    }
  ...
  /**
   * When calculating control digit with the 2,1,2,1... method, the sum for a digit may be > 9. For
   * example in 1978 the 7 will be multiplied by 2, e.g i == 14. 14 is summed as 1+4. In that case
   * 14/10 == 1 and then 14%10 == 4. Result is 5.
   */
